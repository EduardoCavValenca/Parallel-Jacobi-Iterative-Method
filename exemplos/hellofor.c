...
int  main(int argc, char *argv[])  {
	...
	bufrecv = (int *) malloc (2 * sizeof(int));
   	msgtag = 1;
	if ( myrank == 0) {
		...
		for (dest = 1; dest<npes; dest++)
		  MPI_Send(bufrecv, 2, MPI_INT, dest, msgtag, MPI_COMM_WORLD);
		
		msgtag = 2;
		printf("Receiving msgs from slaves: ");
		for (src = 1; src < npes; src++) {
			MPI_Recv(bufrecv, 2, MPI_INT, MPI_ANY_SOURCE, msgtag, MPI_COMM_WORLD, &status);
			printf("%d  %d", bufrecv[0], bufrecv[1]);
		}
		printf("\n");
	}
	else {
		...
		bufsend = (int *) malloc (2 * sizeof(int));
		src = dest = 0;
		MPI_Recv(bufrecv, 2, MPI_INT, src, msgtag, MPI_COMM_WORLD, &status);
        printf("%d  %d", bufrecv[0], bufrecv[1]);
		msgtag = 2;
		//sprintf(bufsend, "%s from %d", bufrecv, myrank);
		MPI_Send(bufsend, 2, MPI_INT, dest, msgtag, MPI_COMM_WORLD);
		free(bufsend);
	}
	free(bufrecv);
	fflush(0);
